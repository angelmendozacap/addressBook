...
@section('content')
  <div class="mx-auto h-full flex justify-center items-center bg-gray-300">
    <div class="w-96 bg-blue-900 rounded-lg shadow-xl p-6">
      ...
      <h1 class="text-white text-3xl pt-8">Únete a Nosotros</h1>
      <h2 class="text-blue-300">Ingresa tu información debajo</h2>
      <form method="POST" action="{{ route('register') }}">
        @csrf

        <div class="relative pt-8">
          <label for="name" class="uppercase text-blue-500 text-xs font-bold absolute pl-3 pt-2">Nombre Completo</label>

          <div>
            <input id="name" type="text" class="pt-8 w-full rounded p-3 bg-blue-800 text-gray-100 outline-none focus:bg-blue-700" name="name"
                   value="{{ old('name') }}" autocomplete="name" autofocus placeholder="Tu Nombre">

            @error('name')
            <span class="text-red-600 text-sm pt-1"><strong>{{ $message }}</strong></span>
            @enderror
          </div>
        </div>

        <div class="relative pt-3">
          <label for="email" class="uppercase text-blue-500 text-xs font-bold absolute pl-3 pt-2">Correo Electrónico</label>
          ...
          <div>
            <input id="email" type="email" class="pt-8 w-full rounded p-3 bg-blue-800 text-gray-100 outline-none focus:bg-blue-700" name="email"
                   value="{{ old('email') }}" autocomplete="email" placeholder="tu@email.com">
        ...
        <div class="relative pt-3">
          <label for="password" class="uppercase text-blue-500 text-xs font-bold absolute pl-3 pt-2">Contraseña</label>
        ...
        <div class="relative pt-3">
          <label for="password-confirm"
                 class="uppercase text-blue-500 text-xs font-bold absolute pl-3 pt-2">Confirmar Contraseña</label>

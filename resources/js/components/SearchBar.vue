<template>
  <div>
    ...
    <div class="relative z-10">
      ...
      <input type="text" class="w-64 mr-6 bg-gray-200 border border-gray-400 pl-8 pr-3 py-1 rounded-full text-sm focus:outline-none focus:border-blue-500 focus:shadow focus:bg-gray-100" placeholder="Buscar..." id="searchTerm" v-model="searchTerm" @input="search" @focus="focus = true">

      <div v-if="focus" class="absolute bg-blue-900 text-white rounded-lg p-4 w-96 right-0 mr-6 mt-2 shadow z-20">
        <div v-if="!results.length">No se encontraron resultados para '{{ searchTerm }}'.</div>
    ...
    methods: {
      search: _.debounce(async function(e) {
        try {
          ...
          const res = await axios.post('/api/search', { searchTerm: this.searchTerm})
